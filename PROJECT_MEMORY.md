...
|------|-----------|---------------|--------|
| June 13, 2026 | **BUG-031 re-fix** — First fix (`.update()` inside rebuild functions) caused "row control must be added to the page first" because initial rebuild runs during construction before row is mounted. Moved `.update()` to user-initiated handlers only: `_select_global()`/`_toggle_cat_chip()` call `cats_row.update()` after rebuild; edit dialog lambdas call `edit_cats_row.update()` as last tuple expression. Initial rebuild calls unmodified. | `views/settings.py`, `PROJECT_MEMORY.md` | Complete |
| June 13, 2026 | **BUG-031** — Tag Master multi-category chip visuals do not live-update. Root cause: `_rebuild_cats_chips()` and `_rebuild_edit_chips()` modify `controls` list without calling `.update()`. Fix: added `cats_row.update()` and `edit_cats_row.update()` after rebuild. Also fixed Edit/Delete button cropping: removed `height=30` constraint (below natural content height), increased card bottom padding from 6 to 10. | `views/settings.py`, `PROJECT_MEMORY.md` | Complete (refined) |
